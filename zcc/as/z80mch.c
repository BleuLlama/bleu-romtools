...
struct mne *mp;
{
	...
	/* like JP but becomes JR if target close enough */
	case S_J:
		if ((v1 = admode(CND)) != 0) {
			v1 &= 0xFF;	/* mask off cond flag bit */
			v1 <<= 3;
			comma();
		} else
			v1 = 0x100;	/* > 0xFF means non-cond */
		expr(&e2, 0);
		if (e2.e_mode != S_USER)
			rerr();
		/* try for a JR first, assume worst case, JP, on first pass */
		if (e2.e_base.e_ap != NULL && e2.e_base.e_ap == dot.s_area) {
			/* -2, different from JR code because before outtab */
			v2 = e2.e_addr - dot.s_addr - 2;
			/* only NZ, Z, NC and C are legal for JR */
			if ((v1 <= 0x18 || v1 > 0xFF) && v2 >= -128 && v2 <= 127) {
				outab(v1 > 0xFF ? 0x18 : 0x20 + v1);
				outab(v2);
				pjumpused = 1;
				break;
			}
		}
		outab(v1 > 0xFF ? 0xC3 : 0xC2 + v1);
		outrw(&e2, 0);
		break;
