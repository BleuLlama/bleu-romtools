 ...
 * src/unix/fronthlp.c
 *
 *  This file was extracted from MAME source for version 0.65.1 of XMAME
 *
 *  search for "JJJJ" to find my modifications
 */
...
struct rc_option frontend_list_opts[] = {
   ...
   { "listwrongfps",	"lwf",			rc_set_int,	&list,
    ...
    /* JJJJJJ */
   { "dumpgdrivers",	"dgd",			rc_set_int,	&list,
     NULL,		LIST_DUMP_GDRIVERS,	0,		NULL,
     "Dump out the Turaco Graphics Driver INI files" },
...
/* Turaco_Dump_Array
 *
 *   dump out an array.  useful for the graphics layout and palette structs
 */
void
Turaco_Dump_Array( FILE * ofile, int count, int * a )
{
    int x;
    for( x=0 ; x<count ; x++ )
    {
	fprintf( ofile, "%3d ", a[x] & 0x00ffff );
    }
    fprintf( ofile, "\n" );
}
 ...
 * Turaco_Dump_Driver
 * 
 *   dumps out the graphics driver for Turaco
 */
void Turaco_Dump_Driver( int i )
{
    char groupname[JBUF];
    char dirname[JBUF];
    char filename[JBUF];
    char driverpath[JBUF];
    char buf[JBUF];
    ...
    /* determine the source file short name */
    /* they are stored as: "src/drivers/FILENAME.c" */
    {
	int c, d, m, l;
	/* skip to just past the second slash */
	if( drivers[i]->source_file )
	{
	    l = strlen( drivers[i]->source_file );
	    m = 0;
	    for( c=0 ; c<l && m!=2 ; c++ )  
	    {
		if( drivers[i]->source_file[c] == '/' )  m++;
	    }

	    /* copy over until the dot. */
	    for( d=0 ;
		 d<(l-c) && d<JBUF && (drivers[i]->source_file[d+c] != '.') ; 
		 d++ )
	    {
		groupname[d] = drivers[i]->source_file[d+c];
	    }
	    groupname[d] = '\0';
	} else {
	    strcpy( groupname, "misc" );
	}
    }

    /* make it CVS friendly */
    if( !strncasecmp( groupname, "cvs", JBUF ))
    {
	strncat( groupname, "_", JBUF );
    }
    
    /* determine if we should even continue... */
    expand_machine_driver(drivers[i]->drv, &drv);

    if(    (drv.video_attributes & VIDEO_TYPE_VECTOR )  /* it's a vector game */
	|| (!drv.gfxdecodeinfo)                         /* no gfx decodes */
    )
	return;

    /* count up the number of banks */
    {
	struct GfxDecodeInfo *gdi = drv.gfxdecodeinfo;
	ndecodes = 0;
	while( gdi->memory_region != -1 )
	{
	    ndecodes++;
	    gdi++;
	}
    ...
    /* make the directory for the driver ( 755 )*/
#ifdef JDEBUG
    if( 0 ) 
#endif
    {
	int dirflags =    (S_IRUSR | S_IWUSR | S_IXUSR)
			| (S_IRGRP           | S_IXGRP)
			| (S_IROTH           | S_IXOTH) ;

	/* make the root driver directory */
	mkdir( "turaco", dirflags );


	/* then make the group directory */
	snprintf( dirname, JBUF, "turaco/%s", groupname );
	mkdir( dirname , dirflags );
    }

    /* for debug. only dump one rom */
#ifdef JDEBUG
    if( strcmp( drivers[i]->name, "mspacmab" ))
	return;
#endif

    /* open the output file, and dump out the driver */
    snprintf( driverpath, JBUF, "turaco/%s/%s.ini", 
				groupname, drivers[i]->name );
    printf( "%4d>> %s\n", ndumped++, driverpath );

#ifdef JDEBUG
    outfile = stdout_file;
#else
    outfile = fopen( driverpath, "w" );
#endif

    {

	/* generic header */
	fprintf( outfile, "#\n# Turaco graphics driver file\n" ); 
	fprintf( outfile, "#\n# Generated by MAME 0.65.1\n" ); 
	fprintf( outfile, "# Modified by Jerry Carpenter\n" ); 
	fprintf( outfile, "# \t\tturacocl@umlautllama.com\n" ); 
	fprintf( outfile, "# \t\thttp://www.cis.rit.edu/~jerry/Software/turacoCL\n" ); 
	fprintf( outfile, "#\n\n" );
	
	/* Turaco header (this is new)*/
	fprintf( outfile, "[Turaco]\n" );
	fprintf( outfile, " FileVersion = 1.0\n" );
	fprintf( outfile, " DumpVersion = 2\n" );
	fprintf( outfile, "      Author = Jerry / MAME 0.65.1 Dump\n" );
	fprintf( outfile, "         URL = http://www.cis.rit.edu/~jerry/Software/turacoCL\n" );
	fprintf( outfile, "\n" );

	/* general information */
	fprintf( outfile, "[General]\n" );
	fprintf( outfile, "        Name = %s\n", drivers[i]->name );
	fprintf( outfile, "    Grouping = %s\n", groupname );
	...
	{
	    fprintf( outfile, "     CloneOf = %s\n", 
			drivers[i]->clone_of->name );
	} else {
	    fprintf( outfile, "     CloneOf =\n" );
	}
	fprintf( outfile, "\n" );


	/* graphics meta information */
	fprintf( outfile, "[Layout]\n" );
	fprintf( outfile, " GfxDecodes = %d\n", ndecodes );
	fprintf( outfile, "\n" );
	...
	if( drivers[i]->rom );
	{
	    ...
	    fprintf( outfile, "[GraphicsRoms]\n" );

	    for (region = rom_first_region(drivers[i]); 
	 	 region;
		 region = rom_next_region(region))
	    {
		for (rom = rom_first_file(region); 
                    rom;
                    rom = rom_next_file(rom))
		{
		    for (chunk = rom_first_chunk(rom); 
			 chunk;
			 chunk = rom_next_chunk(chunk))
		    {

			type = ROMREGION_GETTYPE( region );

			if(    (type >= REGION_GFX1)
			    && (type <= REGION_GFX8)
			)
			{
			    /* CONTINUE blocks not handled entirely 
				correctly yet, but this should get
				it working for now.
			    */
			    if( ROMENTRY_ISCONTINUE( chunk ))
			    {
				strcpy( buf, "__CONTINUE__" );

			    } else if( ROMENTRY_ISSPECIAL( chunk )) {
				strcpy( buf, "__SPECIAL__" );

			    } else {
				strcpy( buf, ROM_GETNAME( chunk ));
			    }

			    length = ROM_GETLENGTH( chunk );
			    if( ROMENTRY_ISSPECIAL( chunk ))
			    {
				/* continue blocks move all over the place */
				offset = ROM_GETOFFSET( chunk );
			    }
			    fprintf( outfile, "Rom%d = %5d %5d %s\n", 
				    c++, offset, length,  buf );

			    /* since we're doing graphics differently */
			    offset += length;
			}
		    }
		}
	...
	/* Graphics Decode Bank information */
	{
	    /* count up graphics decodes */
	    int x = 1;
	    struct GfxDecodeInfo *gdi = drv.gfxdecodeinfo;
	    while( gdi->memory_region != -1 )
	    {
		fprintf( outfile, "[Decode%d]\n", x++ );
		fprintf( outfile, "        start = %d\n", gdi->start );
		fprintf( outfile, "   colorstart = %d\n", 
						gdi->color_codes_start );
		fprintf( outfile, "   colorcodes = %d\n",
						gdi->total_color_codes );

		/* while mame doesn't support multiple banks to be at 
		    different orientations, we will.  HA! */
		fprintf( outfile, "  orientation = %d\n", 
					drivers[i]->flags & 0x07);
		...
		if( gdi->gfxlayout )
		{
		    ...
		    struct GfxLayout *gl = gdi->gfxlayout;

		    fprintf( outfile, "        width = %d\n", gl->width );
		    fprintf( outfile, "       height = %d\n", gl->height );
		    if( IS_FRAC(gl->total) )
		    {
			/* we don't handle these properly yet. */
			region_length = 8 * 
				memory_region_length(gdi->memory_region);   


                        gltotal = (int) (
				  (float)region_length
				/ (float)gl->charincrement 
				* (float)FRAC_NUM(gl->total) 
				/ (float)FRAC_DEN(gl->total)
				);

			fprintf( outfile, "        total = %d\n", gltotal );
			fprintf( outfile, "        ERROR = FRAC not supported yet.\n" );

		    } else {
			fprintf( outfile, "        total = %d\n", gl->total );
		    }

		    fprintf( outfile, "       planes = %d\n", gl->planes );

		    if( gl->planes > maxplanes )   maxplanes = gl->planes;

		    fprintf( outfile, " planeoffsets = " );
		    Turaco_Dump_Array( outfile, gl->planes, gl->planeoffset );

		    fprintf( outfile, "     xoffsets = " );
		    Turaco_Dump_Array( outfile, gl->width, gl->xoffset );

		    fprintf( outfile, "     yoffsets = " );
		    Turaco_Dump_Array( outfile, gl->height, gl->yoffset );

		    fprintf( outfile, "charincrement = %d\n", 
						gl->charincrement );
		}
	...
	/* due to the fact that we need the ROMs (or at least the 
	   color PROMs, ) we cannot initialize this as we'd like, so 
	    for now, just drop in this faux palette. 
	*/

	/* Palettes */  /* for reference only! */
	fprintf( outfile, "[Palette]\n" );
	{
	    int c, col;
	    int ncolors = 1 << maxplanes;

	    /* hardcode a single palette for now. */
	    fprintf( outfile, "Palette1 = %2d ", ncolors );
	    c = 0;
	    while( c != ncolors ) 
	    {
		col = (int)((float)c/(float)(ncolors-1) * 255.0);
		fprintf( outfile, " %d %d %d ", col, col, col);
		c++;
	    }
	    fprintf( outfile, "\n");
	}
...
/* dumpgdrivers     */ ""
   ...
   for (i=0;drivers[i];i++)
   {
              ...
              !strwildcmp(gamename, drivers[i]->name) )
         {
            ...
            {
		/*** JJJJJJ ***/
		case LIST_DUMP_GDRIVERS:
		    Turaco_Dump_Driver( i );
		    break;
